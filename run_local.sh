...
start_runtime() {
  ...
  if [[ -n "${SMARTHOME_LOG_LEVEL:-}" ]]; then
    env_args+=("SMARTHOME_LOG_LEVEL=${SMARTHOME_LOG_LEVEL}")
  fi
  if [[ -n "${SMARTHOME_DEBUG_FLOW:-}" ]]; then
    env_args+=("SMARTHOME_DEBUG_FLOW=${SMARTHOME_DEBUG_FLOW}")
  fi
  if [[ -n "${SMARTHOME_EVENT_BUS_MAX_PER_TOPIC:-}" ]]; then
    env_args+=("SMARTHOME_EVENT_BUS_MAX_PER_TOPIC=${SMARTHOME_EVENT_BUS_MAX_PER_TOPIC}")
  fi
...
Optional env:
  ...
  SMARTHOME_REDIS_URL
  SMARTHOME_LOG_LEVEL, SMARTHOME_DEBUG_FLOW, SMARTHOME_EVENT_BUS_MAX_PER_TOPIC
