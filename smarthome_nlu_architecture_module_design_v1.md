...
### 1.1 端到端数据流
用户说话 → ASR 转文字 → NLU 意图识别（主路 / 兜底） → 结构化 JSON 意图 → ha-mcp 工具调用 → Home Assistant REST/WebSocket → 设备执行 → 结果 TTS 播报。整条链路运行时完全本地，用户隐私数据不出局域网。
...
4. **数据飞轮**：ha-mcp 执行反馈（成功/失败）反哺 NLU 难例队列，持续提升。
...
### 2.2 核心槽位与 ha-mcp 实体映射

| 槽位 | 类型 | 示例值 | ha-mcp 映射逻辑 |
|---|---|---|---|
...
### 2.3 NLU → ha-mcp 接口约定
NLU 层输出标准 JSON，ha-mcp 适配器将其映射为 MCP 工具调用。两层通过内部 HTTP 接口解耦，NLU 层不感知底层 HA 协议：
...
| 执行层 | ha-mcp Server | MCP 协议桥接，82 个 HA 工具 | 边缘服务器 |
...
Fields (Hash):
  last_intent        # 上一意图
  last_device_id     # 焦点设备 entity_id（来自 ha-mcp 执行结果）
  last_device_type   # 焦点设备类型
  last_location      # 焦点房间
  last_attribute     # 焦点属性
  last_ha_result     # ha-mcp 最近执行结果（成功/失败）
  turn_count         # 轮数
...
| 意图修正 | 不对，我说的是卧室 | DST 更新 location，重新调用 ha-mcp |
| ha-mcp 失败后恢复 | 刷新一下，再尝试 | ha_search_entities 重新查找，重试执行 |
...
| 输出 | 候选实体列表 |
| 依赖 | ha-mcp 搜索工具 |
| 索引策略 | 启动时从 ha-mcp 拉取全量实体，建立内存模糊索引，5 分钟刷新 |
| 搜索策略 | 三级：Levenshtein/Jaro-Winkler 模糊匹配 → 精确子串匹配 → 部分匹配兜底 |

```python
# 启动时拉取全量实体，建立本地索引
entities = await ha_mcp.call('ha_get_all_entities')
...
async def execute_with_idempotency(user_id, intent, ha_mcp_client,
                                   dedup_window_sec=30):
    key = make_idempotency_key(user_id, intent)
    cached = await redis.get(key)
    if cached:
        return json.loads(cached)  # 直接返回首次结果
    result = await ha_mcp_client.call_service(intent)
...
| 场景 | 重试次数 | 退避策略 | 最终降级 |
|---|---|---|---|
| ha-mcp HTTP 超时 | 3 次 | 50ms / 150ms / 500ms 指数退避 | TTS 告知失败，入难例队列 |
| ha-mcp 实体未找到 | 1 次 | ha_search_entities 重新模糊搜索 | TTS 询问用户确认设备名 |
| HA 服务调用失败 | 2 次 | 100ms / 300ms | TTS 告知失败，入难例队列 |
| Student 推理超时 | 0 次 | 直接降级规则引擎 | — |

### 4.9 `ha-mcp-adapter` — MCP 适配层
...
| 输出 | 结构化执行响应 |
| 依赖 | ha-mcp HTTP |
| 传输模式 | HTTP 模式（ha-mcp-web），端口 9583，内网直连 |
...
**NLU 服务通信：**
```python
HA_MCP_BASE_URL = 'http://homeassistant.local:9583'

class HaMcpClient:
    async def call(self, tool_name: str, params: dict) -> dict:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.post(
                f'{HA_MCP_BASE_URL}/tools/{tool_name}',
                json=params
            )
            return resp.json()
```
...
| tool_name | 字符串 | 调用的 ha-mcp 工具 | ha_call_service |
...
async def execute_intent(nlu_result, context):
    ...
    if 'device_type' in slots:
        ...
        entities = await ha_mcp.call('ha_search_entities',
    ...
    result = await ha_mcp.call('ha_call_service', {
...
| D. 持续迭代 | 难例挖掘（含 ha-mcp 执行失败反馈）+ 增量蒸馏 + Student 晋升 |
...
| ha-mcp 执行反馈 | 设备执行失败的话语 | 动态增加 | 最真实的错误分布 |
...
| ha-mcp 执行失败 | entity_not_found / 403 | 回溯重标注 entity_id 映射 | 最高 |
...
5. `executor` 调用 `ha-mcp-adapter` 执行，并通过 WebSocket state_changed 做状态验证。
...
| ha-mcp Server | 边缘服务器 / HA Add-on | ha-mcp HTTP 模式（:9583） | 内存 256MB |
...
| uvx 快速启动 | `uvx ha-mcp[web]` 运行 HTTP 模式（:9583） |

### 9.3 节点划分
1. **网关节点**：`api-gateway`、`nlu-main`、`dst-service`。
2. **边缘算力节点**：`nlu-fallback`、`executor`、`ha-mcp-adapter`。
3. **本地 HA 节点**：Home Assistant + ha-mcp。
...
| 故障场景 | 降级方案 |
|---|---|
| ha-mcp 不可达 | 降级为直接调用 HA REST API（/api/services/{domain}/{service}） |
| Student 服务超时（> 800ms） | 降级为关键词规则引擎 |
| Redis 不可达 | 单轮无上下文运行，主路阈值临时降为 0.50 |
| ha-mcp 实体未找到 | TTS 询问用户确认设备名，同时入难例队列 |
| HA 重启期间 | ha-mcp 自动重连，队列化请求等待恢复 |
...
- ha-mcp Add-on 模式：128-bit 熵路径 + Supervisor 自动鉴权，无需明文 Token。
- ENABLED_TOOL_MODULES 最小权限原则：仅启用业务所需工具模块。
- 破坏性操作（删除自动化、恢复备份）需用户二次确认。
- 本地网络隔离：ha-mcp 仅监听局域网 IP，不暴露公网。
...
async def guarded_call(tool_name, params, user_role):
    ...
    return await ha_mcp.call(tool_name, params)
```
...
| ha-mcp 集成测试 | 覆盖 5 类意图端到端设备执行 | 执行成功率 > 98% | 阻断发布 |
...
| 成长期 | 5~12 周 | +8,000 条难例（含 ha-mcp 反馈） | +10,000 条增量 | 主路 92%，兜底 90% |
...
| ha-mcp 实体匹配失败 | 高 | 中 | 维护设备别名词典；ha-mcp 模糊搜索三级兜底 |
| HA 版本升级导致 API 变化 | 中 | 低 | ha-mcp 活跃维护（weekly release），跟踪 CHANGELOG |
| 本地 GPU 资源不足 | 高 | 低 | 预设 CPU 方案（GGUF），接受延迟 < 1500ms |
| Teacher API 成本过高 | 中 | 中 | 一次性生成离线，后续仅用于难例重标注（每周 < $5） |
| 新设备覆盖缺失 | 中 | 高 | ha-mcp ha_search_entities 自动发现；维护实时词表 |
| 模型更新回归 | 高 | 中 | 强制 A/B 测试；自动回滚；保留 3 个历史版本 |
| ha-mcp Token 泄露 | 高 | 低 | 仅内网访问；Add-on 模式无明文 Token；定期轮换 |
...
| Phase 2 ha-mcp 集成 | 第 2~5 周 | ha-mcp 部署，基础设备控制可用 | ha-mcp Add-on + NLU 适配器 |
...
| Phase 7 完整集成 | 第 11~14 周 | NLU + 兜底 + ha-mcp + DST 联调 | 完整系统 v3.1 |
| Phase 8 数据飞轮 | 第 13 周起 | ha-mcp 反馈闭环，首次 Student 晋升 | 增量蒸馏 MLOps 流水线 |

> **Phase 2 优先级说明**：ha-mcp 集成提前到 Phase 2（与主路并行），原因：
> 1. ha-mcp 接入后可立即从执行反馈中获取真实难例数据，显著提升蒸馏数据质量。
> 2. ha-mcp 安装简单（Add-on 一键安装），不阻塞 NLU 开发。
...
| ha-mcp | Home Assistant MCP Server，开源项目，提供 82 个 HA 控制工具 |
...
| Long-Lived Token | HA 长期访问令牌，用于 ha-mcp 身份认证 |
